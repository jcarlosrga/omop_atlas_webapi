...
FROM
    OMOP_CDM.person p1
        left join OMOP_CDM.provider pr1
...
FROM
    OMOP_CDM.person p1
        left join OMOP_CDM.location l1
...
FROM
    OMOP_CDM.person p1
        left join OMOP_CDM.care_site cs1
...
select
    ...
    COUNT(p1.person_id) as count_value
from OMOP_CDM.person p1
         inner join (select person_id, MIN(observation_period_start_date) as index_date from OMOP_CDM.observation_period group by PERSON_ID) op1
...
from OMOP_CDM.person p
         JOIN OMOP_CDM.observation_period op on p.person_id = op.person_id
...
from OMOP_CDM.observation_period op
       ) A
   where rn = 1;
...
from OMOP_CDM.observation_period op
           ...
           JOIN OMOP_CDM.person p on op.person_id = p.person_id
   where op.rn = 1
;
...
from temp.ymd
where obs_year >= (select min(YEAR(observation_period_start_date)) from OMOP_CDM.observation_period) and obs_year <= (select max(YEAR(observation_period_start_date)) from OMOP_CDM.observation_period);
...
FROM
       OMOP_CDM.observation_period op
           CROSS JOIN
       temp.year_ranges yr
   WHERE
...
FROM
       OMOP_CDM.observation_period op1
           join
       (
SELECT
...
SELECT
       ...
       COUNT(distinct op1.PERSON_ID) as count_value
FROM
       (select person_id, COUNT(OBSERVATION_period_start_date) as num_periods from OMOP_CDM.observation_period group by PERSON_ID) op1
...
FROM
       OMOP_CDM.person p1
           inner join (select person_id, MIN(YEAR(OBSERVATION_period_start_date)) as first_obs_year from OMOP_CDM.observation_period group by PERSON_ID) op1
                      on p1.person_id = op1.person_id
...
from OMOP_CDM.observation_period op2, temp.date_keys t2
...
from OMOP_CDM.visit_occurrence vo inner join
     OMOP_CDM.observation_period op on vo.person_id = op.person_id
...
FROM
    OMOP_CDM.provider p
        join OMOP_CDM.visit_occurrence vo
...
FROM
    OMOP_CDM.condition_occurrence co1
        left join OMOP_CDM.person p1
...
FROM
    OMOP_CDM.condition_occurrence co1
        left join OMOP_CDM.observation_period op1
...
FROM
    OMOP_CDM.condition_occurrence co1
        left join OMOP_CDM.provider p1
...
FROM
    OMOP_CDM.condition_occurrence co1
        left join OMOP_CDM.visit_occurrence vo1
...
FROM
            OMOP_CDM.procedure_occurrence po
                JOIN
            OMOP_CDM.observation_period op
            ON
...
FROM
    OMOP_CDM.procedure_occurrence po1
    left join OMOP_CDM.person p1
...
select
             ...
             COUNT(distinct person_id) as count_value
from OMOP_CDM.person p1
             inner join OMOP_CDM.location l1
         on p1.location_id = l1.location_id
         where p1.location_id is not null
...
FROM
    OMOP_CDM.person p1
        inner join OMOP_CDM.care_site cs1
                   on p1.care_site_id = cs1.care_site_id
where p1.care_site_id is not null
...
FROM
    OMOP_CDM.person p1
        inner join
    OMOP_CDM.payer_plan_period ppp1
    on p1.person_id = ppp1.person_id
        ,
    temp_dates_1409 t1
...
FROM
    OMOP_CDM.person p1
        inner join OMOP_CDM.payer_plan_period ppp1
                   on p1.person_id = ppp1.person_id
...
FROM
    OMOP_CDM.person p1
        inner join (select person_id, COUNT(payer_plan_period_start_date) as num_periods from OMOP_CDM.payer_plan_period group by PERSON_ID) ppp1
                   on p1.person_id = ppp1.person_id
...
FROM
    OMOP_CDM.person p1
        inner join (select person_id, MIN(YEAR(payer_plan_period_start_date)) as first_obs_year from OMOP_CDM.payer_plan_period group by PERSON_ID) ppp1
                   on p1.person_id = ppp1.person_id
...
FROM
    OMOP_CDM.measurement m
        left join OMOP_CDM.person p1 on p1.person_id = m.person_id
where p1.person_id is null
;
...
FROM
    OMOP_CDM.measurement m
        left join OMOP_CDM.observation_period op on op.person_id = m.person_id
...
FROM
    OMOP_CDM.measurement m
        left join OMOP_CDM.provider p on p.provider_id = m.provider_id
...
FROM
    OMOP_CDM.measurement m
        left join OMOP_CDM.visit_occurrence vo on m.visit_occurrence_id = vo.visit_occurrence_id
...
FROM
    (
select
'measurement' as table_name, 'measurement_source_value' as column_name, measurement_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.measurement where measurement_concept_id = 0 group by measurement_source_value
        union
select
'measurement' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.measurement where unit_concept_id = 0 group by unit_source_value
        union
select
'procedure_occurrence' as table_name,'procedure_source_value' as column_name, procedure_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.procedure_occurrence where procedure_concept_id = 0 group by procedure_source_value
        union
select
'procedure_occurrence' as table_name,'modifier_source_value' as column_name, modifier_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.procedure_occurrence where modifier_concept_id = 0 group by modifier_source_value
        union
select
'drug_exposure' as table_name, 'drug_source_value' as column_name, drug_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.drug_exposure where drug_concept_id = 0 group by drug_source_value
        union
select
'drug_exposure' as table_name, 'route_source_value' as column_name, route_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.drug_exposure where route_concept_id = 0 group by route_source_value
        union
select
'condition_occurrence' as table_name, 'condition_source_value' as column_name, condition_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.condition_occurrence where condition_concept_id = 0 group by condition_source_value
        union
select
'condition_occurrence' as table_name, 'condition_status_source_value' as column_name, condition_status_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.condition_occurrence where condition_status_concept_id = 0 group by condition_status_source_value
        union
select
'observation' as table_name, 'observation_source_value' as column_name, observation_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.observation where observation_concept_id = 0 group by observation_source_value
        union
select
'observation' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.observation where unit_concept_id = 0 group by unit_source_value
        union
select
'observation' as table_name, 'qualifier_source_value' as column_name, qualifier_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.observation where qualifier_concept_id = 0 group by qualifier_source_value
        union
select
'payer_plan_period' as table_name, 'payer_source_value' as column_name, payer_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.payer_plan_period where payer_concept_id = 0 group by payer_source_value
        union
select
'payer_plan_period' as table_name, 'plan_source_value' as column_name, plan_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.payer_plan_period where plan_concept_id = 0 group by plan_source_value
        union
select
'payer_plan_period' as table_name, 'sponsor_source_value' as column_name, sponsor_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.payer_plan_period where sponsor_concept_id = 0 group by sponsor_source_value
        union
select
'payer_plan_period' as table_name, 'stop_reason_source_value' as column_name, stop_reason_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.payer_plan_period where stop_reason_concept_id = 0 group by stop_reason_source_value
        union
select
'provider' as table_name, 'specialty_source_value' as column_name, specialty_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.provider where specialty_concept_id = 0 group by specialty_source_value
        union
select
'provider' as table_name, 'gender_source_value' as column_name, gender_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.provider where gender_concept_id = 0 group by gender_source_value
        union
select
'person' as table_name, 'gender_source_value' as column_name, gender_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.person where gender_concept_id = 0 group by gender_source_value
        union
select
'person' as table_name, 'race_source_value' as column_name, race_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.person where race_concept_id = 0 group by race_source_value
        union
select
'person' as table_name, 'ethnicity_source_value' as column_name, ethnicity_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.person where ethnicity_concept_id = 0 group by ethnicity_source_value
        union
select
'specimen' as table_name, 'specimen_source_value' as column_name, specimen_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.specimen where specimen_concept_id = 0 group by specimen_source_value
        union
select
'specimen' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.specimen where unit_concept_id = 0 group by unit_source_value
        union
select
'specimen' as table_name, 'anatomic_site_source_value' as column_name, anatomic_site_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.specimen where anatomic_site_concept_id = 0 group by anatomic_site_source_value
        union
select
'specimen' as table_name, 'disease_status_source_value' as column_name, disease_status_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.specimen where disease_status_concept_id = 0 group by disease_status_source_value
        union
select
'visit_occurrence' as table_name, 'visit_source_value' as column_name, visit_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.visit_occurrence where visit_concept_id = 0 group by visit_source_value
        union
select
'visit_occurrence' as table_name, 'admitting_source_value' as column_name, admitted_from_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.visit_occurrence where admitted_from_concept_id = 0 group by admitted_from_source_value
        union
select
'visit_occurrence' as table_name, 'discharged_to_source_value' as column_name, discharged_to_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.visit_occurrence where discharged_to_concept_id = 0 group by discharged_to_source_value
        union
select
'device_exposure' as table_name, 'device_source_value' as column_name, device_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.device_exposure where device_concept_id = 0 group by device_source_value
        union
select
'death' as table_name, 'cause_source_value' as column_name, cause_source_value as source_value, COUNT(*) as cnt from OMOP_CDM.death where cause_concept_id = 0 group by cause_source_value
    ) a
where cnt >= 1
;
        ...
        FROM
            ...
            (SELECT DISTINCT b.person_id
             FROM (SELECT co.person_id
                   FROM OMOP_CDM.condition_occurrence co
                            JOIN
                        OMOP_CDM.observation_period op
                        ON
                                    co.person_id = op.person_id
...
from
    ...
    (select count(distinct(person_id)) as totalPersons from OMOP_CDM.person) as totalPersonsDb
UNION ALL
select
2004 as analysis_id,
